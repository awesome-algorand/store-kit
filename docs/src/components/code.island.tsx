...
export function ReactStoreCode({ type }: { type: "txnlab" | "algokit" }) {
  ...
  useEffect(() => {
    if (hype) return;
    createRenderer(renderOptions as RehypeExpressiveCodeOptions)
      .then((r) => setHype(r))
      .catch(console.error);
  }, []);
    ...
    hype.ec
      .render({
        code: template({
          ...
          // TODO: If not deployed, enable sync
          actions:
            type === "txnlab"
              ? `  .setManager(manager)\n  .init("Lodash", false)`
              : `  .setDeployer(await algorand.account.fromEnvironment('DEPLOYER'))\n  .init("Lodash", false)`,
        }),
        language: "typescript",
      })
      .then((result) => {
