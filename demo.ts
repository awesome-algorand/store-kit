...
userStream.write(['appId', 'username', 'email', 'avatar', 'password', 'birthdate', 'registeredAt']);
...
mbrStream.write(['appId', 'user-size', 'user-mbr', 'locations-size', 'locations-mbr', 'locations-count', 'total-mbr']);
...
export function createRandomUser() {
  ...
  userStream.write([data.appId, data.username, data.email, data.avatar, data.password, data.birthdate, data.registeredAt]);
  return data
}
...
export function createRandomLocation() {
  ...
  locationStream.write([data.appId, data.userId, data.country, data.city, data.state]);
  return data;
}
...
const data = users.reduce((acc, user) => {
  const uLocations = locations.filter((location) => location.userId === user.appId)
  acc[user.appId] = {
    total: BigInt(0),
    user: toMBR(user),
  ...
  const locationMbr = acc[user.appId].locations.reduce((acc, location) => acc + location, 0n)
  acc[user.appId].total = locationMbr + acc[user.appId].user
  console.log(diff(user, {}))
  const userSize= diff({}, user).reduce((acc, kv) => acc + kv[0].length + kv[1].toString().length, 0)
  const locationSize= diff([],uLocations).reduce((acc, kv) => acc + kv[0].length + kv[1].toString().length, 0)
  mbrStream.write([user.appId, userSize, acc[user.appId].user, locationSize, locationMbr, uLocations.length, acc[user.appId].total])
  return acc
...
console.log(`Final Average: ${(bDiff / BigInt(users.length)).microAlgo().algos}`)
