 ...
 */
function getValue(object: {[k: string]: any}, key: string): any {
...
class Hash {
  __data__ = nativeCreate ? nativeCreate(null) : {};
  /**
...
class ListCache{
   ...
   * @param {Array} [entries] The key-value pairs to cache.
   */
  constructor(entries: [key: string, value: any][]) {
    let index = -1,
      length = entries ? entries.length : 0;
 ...
 */
function baseGet(object: {[k: string|symbol]: any}, path: string[]|string): any {
 ...
 */
function castPath(value: any): any[] {
...
function getMapData(map: Map, key: string): any {
 ...
 */
function getNative(object: object, key: string) {
 ...
 */
const stringToPath = memoize(function(string: any) {
